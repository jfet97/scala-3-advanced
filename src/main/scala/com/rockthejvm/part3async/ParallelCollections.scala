...
object ParallelCollections {
  ...
  val aParallelizedIncrementedList: ParSeq[Int] = parList.map(_ + 1) // map, flatMap, filter, foreach, reduce, fold
  /*
    Applicable for
    - Seq
    - Vector
    ...
    - Maps
    - Sets
    Use-case: faster, parallel, processing
   */

  // parallel collection build explicitly
  val aParVecror: ParSeq[Int] = ParVector[Int](1,2,3,4,5,6)
