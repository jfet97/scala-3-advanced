...
object Recap {
  ...
  val anIfExpression: Int = if (aCondition) 42 else 55 // expressions evaluate to a value
  ...
  val anIncrementerList: List[Int] = List(1,2,3).map(anonymousIncrementer) // [2,3,4]
  // map, flatMap, filter
