...
object FBoundedPolymorphism {
  ...
  object Problem {
    ...
    // losing type safety, we got only List[Animal]
  }
  ...
  object FBP {
    ...
    class Cat extends Animal[Cat] { // does Cat extends Animal[Cat]? Yes!
      override def breed: List[Animal[Cat]] = List(new Cat, new Cat)
    }

    class Dog extends Animal[Dog] { // does Dog extends Animal[Dog]? Yes!
      override def breed = List(new Dog, new Dog, new Dog)
    }

    // mess up FBP
    class Crocodile extends Animal[Dog] { // does Dog extends Animal[Dog]? Yes!
  ...
  // FBP + self types
  object FBPSelf {
    trait Animal[A <: Animal[A]] { self: A => // if something extends Animal[T], it must be a T as well
      def breed: List[Animal[A]]
    }
    ...
    class Shark extends Fish {
      override def breed: List[Animal[Fish]] = List(new Cod) // :( List(new Cod) is a List[Animal[Fish]], not a List[Animal[Shark]]
    }
