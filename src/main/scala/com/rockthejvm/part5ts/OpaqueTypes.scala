...
object OpaqueTypes {
  ...
  object SocialNetwork {
    ...
    // inside, we can use Name <-> String interchangeably
    def addFriend(person1: Name, person2: Name): Boolean =
      person1.length == person2.length // we can use the entire String API
  ...
  // 1 - companion objects
  // 2 - extension methods
  val name: Name = Name("Daniel")
  val nameLength = name.length



  // we can create subtype relationships between opaque types
  object Graphics {
